/* ⚠️ ЭТОТ ГЕНЕРАТОР УСТАРЕЛ — НЕ ЗАПУСКАТЬ.
   Живой add-property.html правится напрямую и ушёл вперёд: там таймаут ответа,
   ссылка в кабинет, приём папки ссылкой и защита от «успех выглядит как сбой».
   node build/all.mjs его не собирает. Запуск этого файла затрёт живые правки.
   Если понадобится вернуть сборку — сначала перенести сюда живой файл. */
