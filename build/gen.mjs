  ...
  return objects.map((o, i) => {
    ...
    return {
      ...
      /* 🔴 17.09 Эльнур: «вообще временные акции ставить на витрину не рекомендую!!
         ...
         а не отпугивать». Акция живёт в базе и доступна менеджеру и двойнику,
         но на витрину не выходит: скидка со сроком стареет быстрее, чем мы её меняем,
         и просроченная акция отталкивает сильнее, чем помогает свежая. */
      promo: null,
...
/* 17.09 Эльнур: «временные акции не пишем на сайт, пока не надо». Условие со сроком
   («до 30 сентября», «по 11 сентября включительно», «с 12-го новые цены», «осталось 4 юнита»)
   устаревает молча — у Manor так висело «старые цены до 11.09». На витрину идут только
   постоянные условия: рассрочка, гарантия дохода, мебельный пакет. */
function promoIsTimed(t) {
  return /(^|[^а-яё])(до|по)\s+\d{1,2}(\s*[-–]?\s*(го|е))?\s+(январ|феврал|март|апрел|ма[йя]|июн|июл|август|сентябр|октябр|ноябр|декабр)/i.test(t)
    || /(^|[^а-яё])(до|по)\s+\d{1,2}\.\d{2}/i.test(t)
    || /с\s+\d{1,2}\s*-?\s*го/i.test(t)
    || /(остал|оста[её]т)[а-яё]*\s+(всего\s+)?\d+\s+(юнит|квартир|лот|вилл)/i.test(t)
    || /(ограниченн[а-яё]*\s+(время|срок)|только\s+(до|в)\s+)/i.test(t);
}
...
function objectPage(o, benchmarks, ratesBy, allObjects) {
  ...
  /* 24.09.2026. Сверка показала две беды разом:
     • 19 заголовков объектов длиннее 70 знаков — поиск обрезает хвост с районом;
     • 12 групп страниц с ОДИНАКОВЫМ заголовком: все юниты проекта назывались
       как сам проект («Gardens of Eden · квартира»), и поиск считал их дублями.
     Разводим юниты тем, что у них есть. Спальни и площадь заполнены у немногих,
     поэтому запасной различитель — хвост публичного кода (U1, U2): он и так
     стоит в адресе страницы и наружу открыт по построению.
     Длину держим в 65 знаках: сначала жертвуем брендом, потом словом «Пхукет». */
  const _хвостКода = (function(){
    const м = /-([A-Z]?\d{1,4}|U\d+|[A-Z]\d+)$/.exec(String(pub || ''));
    return м ? м[1] : '';
  })();
  const _различитель = (function(){
    /* 24.09, вторая правка. Сначала различал по спальням и площади, а код брал
       только когда их нет. Но у Clover R1/R2/R3 и Katabello R1/R2 спальни и
       площадь ОДИНАКОВЫЕ — и заголовки снова совпали. Код юнита уникален по
       построению, поэтому он идёт первым и всегда; спальни и площадь добавляем
       сверху, если остаётся место. */
    const сп = (o.bedrooms || '').toString().trim();
    /* В заголовке сотые доли метра — мусор: «57.57 м²». Округляем до целого,
       точная площадь остаётся в карточке. */
    const пл = String(areaLabel(o) || '').replace(/(\d+)[.,]\d+/g, (_, ц) => ц);
    const доп = (сп && пл) ? (сп + ' сп., ' + пл) : (пл || (сп ? сп + ' сп.' : ''));
    if (_хвостКода && доп) return _хвостКода + ' · ' + доп;
    return _хвостКода || доп;
  })();
  ...
  /* Было 200 знаков — поиск показывает около 160 и режет на полуслове.
     49 описаний объектов уезжали в обрез. */
  /* 24.09: у двух объектов описание вышло в 17 и 49 знаков — поиск такое
     не показывает вовсе. Если своего текста мало, дополняем фактами карточки:
     тип, район, площадь. Ничего не выдумываем — только то, что в базе. */
  ...
  const chips = chipRow([
   ...
   по району, тот же, что в блоке ниже. Без слов «до расходов» и «чистыми». */
    /* 16.09 Эльнур: «одинаковые цифры на каждом проекте ничего не говорят, лучше убрать,
...
export function buildSectionPages(indexHtml) {
  const pages = [
    { file: 'buy.html', navId: 'sale', sections: ['sale', 8, 9],
      ...
      /* 24.09: был 71 знак, поиск обрезал бренд. 55 — влезает целиком. */
      title: 'Купить недвижимость на Пхукете | Property Library',
