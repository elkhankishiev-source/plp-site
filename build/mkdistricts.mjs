...
const D=[
 ...
 {ru:'Ката', slug:'kata', in:'Кате', about:
  'Курортный юго-запад с двумя пляжами — Ката и Ката Ной. Живой туристический поток круглый год, устойчивый спрос на аренду.'},
...
let made=[];
for(const d of D){
  /* 24.09.2026. Все восемь районов уезжали в обрез: заголовок до 81 знака,
     описание до 237. Поиск режет на полуслове, и хвост с районом пропадает —
     а район здесь и есть главное слово, по нему нас ищут.
     Заголовок собираем коротко и с районом впереди, бренд отдаём первым, если
     не влезает. Описание — своя фраза про район, обрезанная по границе слова. */
  const title=(function(){
    const полный=`Недвижимость в ${d.in}, Пхукет — купить виллу или квартиру | Property Library`;
    if(полный.length<=65) return полный;
    const безБренда=`Недвижимость в ${d.in}, Пхукет — купить виллу или квартиру`;
    if(безБренда.length<=65) return безБренда;
    return `Недвижимость в ${d.in}, Пхукет — купить`;
  })();
  const desc=(function(){
    const свой=`${d.about} Проверенные объекты в ${d.in}: цены, доходность, сопровождение сделки.`;
    if(свой.length<=160) return свой;
    /* Режем по границе слова, чтобы не обрывать на полуслове, и ставим точку. */
    let к=свой.slice(0,157);
    const п=к.lastIndexOf(' ');
    if(п>100) к=к.slice(0,п);
    return к.replace(/[\s,;:—-]+$/,'')+'…';
  })();
