...
def _():
    ...
    # 23.09.2026, вторая правка за день. Сторож проверял, что токен ВЫДАН, и на
    # этом останавливался — саму ссылку он не открывал ни разу. Из-за этого он
    # проспал, что двойник полгода отдавал клиенту адрес, который про токены не
    # знает ни строки и на любой токен отвечает «Оффер не найден».
    # Проверять надо ровно так, как зовёт клиент: браузер клиента дёргает
    # offer_read с токеном, значит и мы дёргаем offer_read.
    if токен:
        к2, д2 = гет('/rpc/offer_read', {'p_token': токен})
        открылся = (к2 == 200 and isinstance(д2, dict) and д2.get('ok') is not False)
        if not открылся:
            return False, 'токен выдан, но страница по нему не открывается: %s %s' % (к2, str(д2)[:120])
    # Проверка создаёт НАСТОЯЩУЮ строку оффера, и за сутки их
    ...
    return (bool(токен), 'токен выдан и ссылка открывается' if токен else 'токен не выдан')
