...
def таблица(строки, РОЛЬ):
    ...
    шапка = ['Собственник', 'Проект', 'Юнит', 'Спальни', 'Площадь', 'Район',
             'Состояние', 'Цена, ฿', 'Куплено', 'Передача',
    ...
    for имя, к, юниты in строки:
        ...
        for u in юниты:
            ...
            значения = [
                ...
                (дата(u.get('handover_on')) +
                 ((' (' + (('со слов' if u.get('handover_caveat') else '')
                           or u.get('передача_откуда') or '') + ')')
                  if (u.get('handover_caveat') or u.get('передача_откуда')) else ''))
                if u.get('handover_on') else '',
                u.get('не_хватает') or '', u.get('object_id') or '']
