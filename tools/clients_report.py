    ...
    for н, (имя, к, юниты) in enumerate(строки):
        ...
        for r in range(первый, последний + 1):
            for к_и in range(1, len(шапка) + 1):
                ...
                верх = РАМКА if r == первый else None
                # между объектами одного человека — волосяная линия, и только
                # в колонках объекта: имя и контакты стоят объединённой клеткой
                низ = ТОНКАЯ if (r < последний and к_и > ЧЕЛОВЕК) else None
                if верх or низ:
                    c.border = Border(top=верх, bottom=низ)
