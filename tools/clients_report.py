...
def таблица(строки, РОЛЬ):
    ...
    ЧЕЛОВЕК = 1            # вся карточка человека — одна колонка
    ...
    шапка = ['Собственник', 'Проект', 'Юнит', 'Спальни', 'Площадь', 'Район',
    ...
    for имя, к, юниты in строки:
        ...
        for пункт in (к.get('code'),
                      ('+' + str(к['phone'])) if к.get('phone') else None,
                      к.get('email'),
                      None if к.get('вход') else 'кабинет не открыт'):
            if пункт:
                карточка.append(пункт)
        ws.cell(row=первый, column=1, value='\n'.join(карточка))
