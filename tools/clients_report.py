...
def таблица(строки, РОЛЬ):
    ...
    ПЕРЕНОС = {'Собственник и совладелец', 'Почта', 'Состояние', 'Чего не хватает'}
    ПОТОЛОК = {'Собственник и совладелец': 22, 'Почта': 22, 'Проект': 21,
               'Состояние': 18, 'Чего не хватает': 26}
    ...
    # ---- 5. ширины по содержимому, с потолком на болтливые колонки --------
    for i, имя_к in enumerate(шапка, 1):
        длины = [max((len(x) for x in str(ws.cell(row=r, column=i).value or '').split('\n')),
                     default=0) for r in range(1, низ + 1)]
        ws.column_dimensions[get_column_letter(i)].width = min(
            max(max(длины) + 2, 8), ПОТОЛОК.get(имя_к, 20))
