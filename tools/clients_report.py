...
def таблица(строки, РОЛЬ):
    ...
    for имя, к, юниты in строки:
        ...
        контакты = [x for x in (к.get('code'),
                                ('+' + str(к['phone'])) if к.get('phone') else None) if x]
        if контакты:
            карточка.append(' · '.join(контакты))
        if к.get('email'):
            карточка.append(к['email'])
        if not к.get('вход'):
            карточка.append('кабинет не открыт')
