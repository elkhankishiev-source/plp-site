...
def таблица(строки, РОЛЬ):
    ...
    ВЫСОТА_СТРОКИ = 18      # строка объекта
    СТРОКА_ТЕКСТА = 15      # сколько занимает одна строка текста в клетке
    ОТСТУП = 5              # минимальный воздух сверху и снизу полосы
    ряд = 2
    пустые = []
    высоты = []
    for н, (имя, к, юниты) in enumerate(строки):
        ...
        for r in range(первый, последний + 1):
            ...
            for к_и in range(1, len(шапка) + 1):
                ...
                низ = РАМКА if r == последний else None
