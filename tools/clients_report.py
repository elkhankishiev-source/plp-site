...
def таблица(строки, РОЛЬ):
    ...
    # 25.09, Эльнур: «между людьми толстые полосы, внутри своей колонки одного
    # владельца тонкие полоски». Тонкая идёт только по колонкам ОБЪЕКТОВ:
    # объединённую клетку имени она бы перечеркнула.
    # 25.09: было 'hair' — на экране такой линии просто не видно, Эльнур
    # справедливо сказал «вертикальной разметки нет». Ставим 'thin' и цвет
    # потемнее: линия должна читаться, а не угадываться.
    ТОНКАЯ = Side(style='thin', color='B8B0A0')

    wb = Workbook()
    ws = wb.active
    ws.title = 'Собственники'
    # 25.09, Эльнур: «владелец, совладелец — в одной колонке». Отдельная колонка
    # совладельца была нужна, пока жена Александра стояла на трёх объектах из
    # девяти; теперь она на всех девяти, и у остальных совладелец тоже покрывает
    # все объекты человека — значит его место в полосе человека, а не в строке.
    шапка = ['Собственники', 'Код', 'Телефон', 'Почта', 'Кабинет',
             'Проект', 'Юнит', 'Спальни', 'Площадь', 'Район',
    ...
    for c in ws[1]:
        ...
        # 25.09, Эльнур: «полосы не везде прорисованные». Шапка вообще стояла
        # без рамок — разлиновка начиналась только со второй строки и выглядела
        # обрубленной сверху.
    ...
    for н, (имя, к, юниты) in enumerate(строки):
        ...
        # 25.09, Эльнур: «одно имя не зажимай в узкую полоску, одна строка отступ
        # сверху и снизу». У человека с одним объектом полоса была в одну строку.
        # Пустая строка сверху и снизу — имя встаёт по центру своей полосы.
        #
        # ВАЖНО про объединённые клетки: значение живёт только в ЛЕВОЙ ВЕРХНЕЙ
        # клетке объединения. Я сначала писал имя в первую строку ОБЪЕКТА, а
        # объединение начиналось строкой выше — имя попадало в неглавную клетку,
        # пропадало при записи, и Excel объявлял файл повреждённым. Поэтому
        # данные человека пишем в строку-отступ, с которой объединение и
        # начинается, а строки объектов оставляем по этим колонкам пустыми.
        отступы = [ряд]
        ряд += 1
        for u in юниты:
            сп = u.get('спальни')
            for к_и, зн in enumerate([
                    ...
                    u.get('object_id') or ''], ЧЕЛОВЕК + 1):
                ws.cell(row=ряд, column=к_и, value=зн)
            ряд += 1
        отступы.append(ряд)
        ряд += 1
        последний = ряд - 1

        # 25.09. Объединяем НЕ ЗДЕСЬ, а после того, как расставлены рамки.
        # Причина: запись .border в неглавную клетку объединения openpyxl
        # переносит на главную — и закрывающая линия полосы оказывалась под
        # ИМЕНЕМ, а низ полосы оставался открытым. Эльнур увидел это на
        # последней полосе: «наоборот не хватает, в самом конце у Юрия».
        if последний > первый:
            объединить.append((первый, последний))
        for к_и, зн in enumerate([
                имя + (('\n' + '\n'.join(совладельцы)) if совладельцы else ''),
                к.get('code') or '',
                ('+' + str(к['phone'])) if к.get('phone') else '',
                к.get('email') or '',
                'да' if к.get('вход') else ''], 1):
            ws.cell(row=первый, column=к_и, value=зн)

        # Всё прочерчено: по вертикали тонкие линии между колонками, по
        # горизонтали — толстая на границе людей и тонкая между объектами.
        for r in range(первый, последний + 1):
            строка_объекта = r not in отступы
            for к_и in range(1, len(шапка) + 1):
                c = ws.cell(row=r, column=к_и)
                c.fill = PatternFill('solid', fgColor=тон)
                верх = РАМКА if r == первый else (
                    ТОНКАЯ if (строка_объекта and (r - 1) not in отступы and к_и > ЧЕЛОВЕК) else None)
                # 25.09. Полосу закрываем снизу и по нижней строке-отступу, и по
                # последней строке объекта: пустую строку часть просмотрщиков
                # рисует без рамки, и полоса выглядела незакрытой — Эльнур
                # заметил это на самой нижней полосе.
                низ = РАМКА if (r == последний or (строка_объекта and r + 1 in отступы)) else None
                c.border = Border(top=верх, bottom=низ, left=ТОНКАЯ, right=ТОНКАЯ)
        пустые.extend(отступы)
        ws.cell(row=первый, column=1).font = Font(bold=True)
        # 25.09, Эльнур: «в одну строчку вмещается 1 строка имени, надо сделать
        # три и имя посредине». У человека с одним объектом полоса была
        # 5+18+5 = 28 пунктов — полторы строки текста. Считаем, сколько строк
        # реально нужно имени и совладельцам, и добираем высоту отступами.
        строк_имени = max(3, len(совладельцы) + 1)
        нужно = строк_имени * СТРОКА_ТЕКСТА
        есть = len(юниты) * ВЫСОТА_СТРОКИ
        добор = max(ОТСТУП, int((нужно - есть) / 2) + 1)
        высоты.append((отступы, добор))

    for первый_п, последний_п in объединить:
        for к_и in range(1, ЧЕЛОВЕК + 1):
            ws.merge_cells(start_row=первый_п, start_column=к_и,
                           end_row=последний_п, end_column=к_и)
    ...
    ПЕРЕНОС = {'Собственники', 'Почта', 'Состояние', 'Чего не хватает'}
    for i, имя_к in enumerate(шапка, 1):
    ...
    for отступы_п, добор in высоты:
        for r in отступы_п:
            ws.row_dimensions[r].height = добор
    for н, (имя, к, юниты) in enumerate(строки):
        pass
