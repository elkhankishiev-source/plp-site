...
def таблица(строки, РОЛЬ):
    ...
    ЧЕЛОВЕК = 5            # колонки человека: имя с совладельцем, код, телефон, почта, кабинет
    ВЫСОТА = 14            # строка объекта
    СТРОКА = 12            # одна строка текста в клетке
    ВОЗДУХ = 2             # минимальный отступ сверху и снизу полосы
    ТОН = ('FFFFFF', 'F4F2EA')

    # 25.09, Эльнур: «пропорции исправь, чтобы всё лаконично выглядело».
    # Восемнадцать колонок — много, и две из них не зарабатывали своё место:
    # «Статус УК» пуст у всех сорока девяти строк, а «Откуда срок» — приписка
    # к дате, ей хватает скобок в той же клетке. Осталось шестнадцать.
    # «Один человек — одна колонка» (его же диктовка) доведено до конца: имя,
    # совладелец, код, телефон, почта и отметка кабинета живут в ОДНОЙ клетке
    # на всю полосу. Высота полосы и так считается под три строки, они туда
    # помещаются, а лист теряет четыре колонки и полсотни знаков ширины.
    # 25.09, Эльнур: «жду каждый пункт раздельно, не в кашу». Строка
    # «сумма · дата · срок · УК» в одной клетке — именно каша: глазами её надо
    # разбирать. Разношу по отдельным узким колонкам: пусто — значит есть,
    # «нет» — значит не хватает. Срок передачи и его источник тоже развожу.
    # 25.09, Эльнур: «номер телефона отдельно, я же говорил, как ранее ты
    # делал». Код, телефон и почта возвращаются СВОИМИ колонками — как было до
    # того, как я свёл всё в одну клетку. В карточке остаётся только то, что
    # описывает человека: имя и совладелец.
    шапка = ['Собственник и совладелец', 'Код', 'Телефон', 'Почта', 'Кабинет',
             ...
             'Нет суммы', 'Нет даты', 'Нет срока', 'Нет УК', 'Код объекта']
    ВПРАВО = {'Цена, ฿'}
    ЦЕНТР = {'Кабинет', 'Юнит', 'Спальни', 'Площадь', 'Куплено'}
    ...
    ПОТОЛОК = {'Собственник и совладелец': 26, 'Почта': 20,
               'Состояние': 16, 'Код объекта': 19, 'Проект': 34}
    ...
    for имя, к, юниты in строки:
        ...
        for u in юниты:
            ...
            значения = [
                ...
                'нет' if 'сумма' in (u.get('не_хватает') or '') else '',
                'нет' if 'дата' in (u.get('не_хватает') or '') else '',
                'нет' if 'срок' in (u.get('не_хватает') or '') else '',
                'нет' if 'УК' in (u.get('не_хватает') or '') else '',
                u.get('object_id') or '']
        ...
        # 25.09, Эльнур: «зачем ты склеил номер, айди — все пункты раздельно,
        # аккуратно». Код и телефон стояли в одной строке через точку, и клетка
        # читалась как каша. Каждый пункт на своей строке; высота полосы и так
        # считается по числу строк, так что место есть.
        карточка = [имя] + совладельцы
        ...
        полосы.append((первый, последний, объектные, len(карточка)))
    ...
    for семья in (('Юнит', 'Спальни', 'Площадь', 'Район'),
                  ('Куплено', 'Передача'),
                  ('Нет суммы', 'Нет даты', 'Нет срока', 'Нет УК')):
...
Персональные данные. Наружу этот лист не выкладывается.</footer>
    ...
    нехватка = []
    # 25.09: формулировки в представлении укоротились («дата покупки» → «дата»),
    # а этот перечень остался старым — и подпись Дарье назвала одну нехватку из
    # пяти. Держим ровно те слова, что стоят в client_objects_full.
    for к in ('сумма', 'дата', 'срок', 'спальни', 'УК'):
